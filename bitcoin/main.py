...
def privkey_to_pubkey(privkey):
    ...
    if privkey >= N:
        raise Exception("Invalid privkey")
    pubkey = coincurve.PrivateKey.from_int(privkey).public_key.point()
    if f in ['bin', 'bin_compressed', 'hex', 'hex_compressed', 'decimal']:
        return encode_pubkey(pubkey, f)
    else:
        return encode_pubkey(pubkey, f.replace('wif', 'hex'))
